...
void Server::doStartTweet() {
    ...
    while(1) {
        ...
        if(cmd == "tweet") {
            ...
            string op = "tweet " + to_string(tweetlen) + "#" + input + "#" + string(asctime(lctm)) + "#" + string(asctime(utctm)) + "#";
            //cerr << op << "\n";
            ...
            string ownview = string(asctime(lctm)) + " " + to_string(id)+" "+"tweet "+input;
            cout << ownview << endl;
            //cout << "userID: " <<userID << "\n";
            ...
            unordered_set<int> ownblockeduser;

            //Ni Zhang 10/15 11:44 read the dict to find out the users this site wants to block
            if(!dict.Entry.empty()){
                
                for(auto it = dict.Entry.begin(); it != dict.Entry.end(); it++){
                    if((*it).first == userID){
                        ownblockeduser.insert((*it).second);
            ...
            for(auto itfdv = fdv.begin(); itfdv != fdv.end(); itfdv++){
            //for(auto & i : fdv) {
                //cout << "sending objective" <<(*itfdv).second << endl;
                vector<string> NP;
                for(auto it = log.Events.begin(); it != log.Events.end(); it++){
                    //for(int j = 0; j < numSites; j++){ if unblocked could be sent
                    if(!hasRec(timeTable, *it, (*itfdv).second)){
                ...
                n = send((*itfdv).first, sendout, strlen(sendout), 0);
...
void Server::doSomethingWithReceivedData(string& message) {
    ...
    if(hasplnp){
        ...
        for(int i = 0; i < NE.size(); i++){
            ...
            if(cont == "block"){
                ss >> cont;
                dict.Insert(NE[i].userID, stoi(cont));
            }
        }
        ...
        for(int j = 0; j < NE.size(); j++){
            for(auto it = dict.Entry.begin(); it != dict.Entry.end(); ){
                ...
                if(NE[j].op == "unblock "+ to_string((*it).second) && NE[j].userID == (*it).first){
                    ...
                    for(auto logit = log.Events.begin(); logit != log.Events.end();){
                        if((*logit).userID == (*it).first && ((*logit).op == "block "+ to_string((*it).second)||(*logit).op == "unblock "+ to_string((*it).second))){
                            logit = log.Events.erase(logit);
                        }else{
                            logit++;
                        }
                    }
    ...
    for(int i = 0; i < npv.size(); i++){
        log.updateLog(npv[i].op, npv[i].clock, npv[i].userID);
    }
    ...
    if(hasplnp){
        ...
        for(auto it = dict.PL.begin(); it != dict.PL.end(); ){
            ...
            if(good == false){
                it = dict.PL.erase(it);//cannot erase on the original dict, or iterator will be wrong
            }else{
                it++;
            }
        }
...
void Server::unblock(string& input) {
    ...
    log.updateLogUnblock(op, clock, userID, unblockID);
    dict.updatePartialLog(op, clock, userID);
...
void Server::view() {
    ...
    for(auto it = log.Events.begin(); it != log.Events.end(); it++){
        ...
        if(content == "tweet"){
            ...
            tt1 >> get_time(&tm1, "%c");
            //tt1.imbue(locale("en_US.utf-8"));
            tweet.local = mktime(&tm1);
            //cout << "pushing local time" << asctime(&tm1) << endl;
            //cout << "tweet.local" << ctime(&tweet.local) << endl;
            
            istringstream tt3(content);
            tt3 >> get_time(&tm1, "%c");
            //tt1.imbue(locale("en_US.utf-8"));
            tweet.local = mktime(&tm1);
            //cout << "pushing local time" << asctime(&tm1) << endl;
            ...
            tt2 >> get_time(&tm2, "%c");
            tweet.utc = mktime(&tm2);
            //cout << "pushing utc time" << asctime(&tm2) << endl;
            //cout << "tweet.utc" << ctime(&tweet.utc) << endl;
            istringstream tt4(content);
            tt4 >> get_time(&tm2, "%c");
            tweet.utc = mktime(&tm2);
...
void Server::viewdict(){
    for(auto it = dict.Entry.begin(); it != dict.Entry.end(); it++){
        cout << (*it).first << " block " << (*it).second << endl;
    }
}


bool Server::hasRec(vector<vector<int> > timeTable, Event eR, int user){
    return timeTable[user-1][eR.userID-1] >= eR.clock;
}
