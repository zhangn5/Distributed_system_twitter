...
void *Server::startTweet(void *args) {
    ...
    while(1) {
        ...
        if(cmd == "tweet") {
            ...
            string op = "tweet " + to_string(tweetlen) + "#" + input + "#" + ctime(localtime) + "#" + asctime(utctm);
            ...
            string ownview = ctime(localtime) + " " + to_string(id)+" "+"tweet "+input;
            cout << ownview << endl;
            
            //construct sending message with timetable ,np and partial log np
            string sending = tmsglen + " " + tmsg + npmsglen +" " + npmsg + plnpmsglen + " " + plnpmsg;
...
void Server::doSomethingWithReceivedData(string& message) {
    ...
    string cont;
    ss >> cont;
    tmsg = message.substr(cont.length() + 1, stoi(cont));
    int start = cont.length() + 1 + stoi(cont)
    stringstream left1(message.substr(start));
    left1 >> cont;
    npmsg = message.substr(start + cont.length() + 1, stoi(cont));
    start += cont.length() + 1 + stoi(cont);
    ...
    for(int i = 0; i < numSites; i++){
        timeTable[userIDi-1][i] = max(timeTable[userIDi-1][i], tmsgTable[userIDk-1][i]);
    }
    for(int i = 0; i < numSites; i++){
...
void Server::unblock(string& input) {
    ...
    log.updateLog(op, clock, userID);
    log.updatePartialLog(op, clock, userID);
...
void Server::view() {
    ...
    for(auto it = log.Events.begin(); it != log.Events.end(); it++){
        ...
        if(content == "tweet"){
            ...
            struct tm tm;
            strptime(content, "%a %b %d %H:%M:%S %Y", &tm);
